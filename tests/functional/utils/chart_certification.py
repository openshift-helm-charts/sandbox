...
"""
    ...
    def check_pull_request_labels(self,pr_number,expect_labels,logger=pytest.fail):
        r = github_api(
        ...
        if expect_labels:
            ...
            if authorized_request or content_ok:
                logger(f"PR{pr_number} authorized request and/or content-ok label found for PR which was not merged")
                return False
        return True
...
class ChartCertificationE2ETestSingle(ChartCertificationE2ETest):
    ...
    # expect_merged: boolean representing whether the PR should be merged
    def check_pull_request_labels(self, expect_labels: bool):
        super().check_pull_request_labels(self.secrets.pr_number, expect_labels, pytest.fail)
