Feature: Report only submission with provider control settings
  ...
  Examples:
  | report_path               | index_file                        | chart_path                  |
  | tests/data/report.yaml    | unpublished-certified-charts.yaml | tests/data/vault-0.17.0.tgz |


  Scenario Outline: A partner associate submits an error-free report with provider controlled delivery
    Given the vendor <vendor> has a valid identity as <vendor_type>
    And provider control is set to <provider_control_owners> in the OWNERS
    And an error-free report is used in <report_path>
    And provider control is set to <provider_control_report> in the report
    ...
    Examples:
      | vendor_type  | vendor    | provider_control_owners | provider_control_report |
      | partners     | hashicorp | true                    | true                    |
  ...
  Scenario Outline: A partner associate submits an error-free report and chart with provider controlled delivery
    ...
    Examples:
      ...
      | partners     | hashicorp | true                    | true                    | OWNERS file and/or report indicate provider controlled delivery but pull request is not report only. |


  Scenario Outline: A partner associate submits an error-free report with inconsistent provider controlled delivery setting
    Given the vendor <vendor> has a valid identity as <vendor_type>
    And provider delivery control is set to <provider_delivery_owners> in the OWNERS file
    And an error-free report is used in <report_path>
    And provider delivery control is set to <provider_delivery_report> in the report
    When the user sends a pull request with the report
    Then the pull request is not merged
    And user gets the <message> in the pull request comment
    ...
    Examples:
    Examples:
      | vendor_type  | vendor    | provider_delivery_owners | provider_delivery_report | message |
