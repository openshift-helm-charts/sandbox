Feature: Chart source submission with report
    ...
    Scenario Outline: [HC-013] A partner or redhat associate submits an error-free chart source with report
    ...
    Scenario Outline: [HC-014] A community user submits an error-free chart source with report
        Given the vendor <vendor> has a valid identity as <vendor_type>
        And an error-free chart source is used in <chart_path> and report in <report_path>
        When the user sends a pull request with the chart and report
