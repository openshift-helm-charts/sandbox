Feature: Chart verifier comes back with a failure
  ...
  Scenario Outline: [HC-005] A partner or community user submits a chart which does not contain a readme file
  ...
  Scenario Outline: [HC-006] A redhat user submits a chart which does not contain a readme file
    Given the vendor <vendor> has a valid identity as <vendor_type>
    And chart source is used in <chart_path>
    And README file is missing in the chart
    When the user pushed the chart and created pull request
