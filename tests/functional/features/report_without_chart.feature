Feature: Report only submission
    ...
    Scenario Outline: [HC-020] A partner or redhat associate submits an error-free report
    ...
    Scenario Outline: [HC-021] A community user submits an error-free report
        Given the vendor <vendor> has a valid identity as <vendor_type>
        And an error-free report is used in <report_path>
        When the user sends a pull request with the report
