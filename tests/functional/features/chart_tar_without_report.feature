Feature: Chart tarball submission without report
    ...
    Scenario Outline: [HC-003] A partner or redhat associate submits an error-free chart tarball
    ...
    Scenario Outline: [HC-004] A community user submits an error-free chart tarball without report
        Given the vendor <vendor> has a valid identity as <vendor_type>
        And an error-free chart tarball is used in <chart_path>
        When the user sends a pull request with the chart
