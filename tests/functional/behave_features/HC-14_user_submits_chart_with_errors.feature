Feature: Chart submission with errors
  ...
  Scenario Outline: [HC-14-001] An unauthorized user submits a chart
    ...
    Examples:
      ...
      | partners     | hashicorp | tests/data/common/vault-0.17.0.tgz | is not allowed to submit the chart on behalf of  | unauthorized |
    
    @redhat @full
    Examples:
      | vendor_type  | vendor    | chart_path                         | message                                          | user         |
      | redhat       | redhat    | tests/data/common/vault-0.17.0.tgz | is not allowed to submit the chart on behalf of  | unauthorized |
    
    @community @full
    Examples:
      | vendor_type  | vendor    | chart_path                         | message                                          | user         |
      | community    | redhat    | tests/data/common/vault-0.17.0.tgz | is not allowed to submit the chart on behalf of  | unauthorized |
