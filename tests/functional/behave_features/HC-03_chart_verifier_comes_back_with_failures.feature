Feature: Chart verifier comes back with a failure
  ...
  Scenario Outline: [HC-03-001] A partner or community user submits a chart which does not contain a readme file
    ...
    Examples:
        ...
        | partners     | hashicorp | tests/data/common/vault-0.17.0.tgz    | Chart does not have a README                            |

    @community @full
    Examples:
        | vendor_type  | vendor    | chart_path                            | message                                                 |
        | community    | redhat    | tests/data/common/vault-0.17.0.tgz    | Community charts require maintainer review and approval |
