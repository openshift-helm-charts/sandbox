...
charts/partners/acme/awesome/0.1.0/awesome-0.1.0.prov
```
...
For a signed chart, if a public key was provided to the chart verifier when generating the report, a digest of the key is included in the report. If a base 64 encoded public key is then included in the OWNERS file a check is made to conform the digest of the key in the OWNERS file matches the key digest in the report, and if the match fails the PR will fail. However, if the key digests match, and there are no other errors when processing the PR a release is generated containing the public key and the report:
...
#### Signed Chart
For a signed chart the submission must include a tarball and a providence file in addition to the report file. In this case, for checking purpose the PR is treated similarly to a [report only submission](#report_for_a_signed_chart). 
...
Signed chart failures can occur when a public pgp key is included in the OWNERS file.
