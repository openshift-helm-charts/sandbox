...
def generate_verify_report(directory, category, organization, chart, version):
    ...
    if "chart-verifier" not in openshift_tools_installer_output:
        msg = "[ERROR] missing 'chart-verifier' binary"
        write_error_log(directory, msg)
        sys.exit(1)
    chart_verifier_binary_path = openshift_tools_installer_output.get("installedPath", None)
    ...
    if src_exists:
        if os.path.exists(report_path):
            out = subprocess.run([chart_verifier_binary_path, "verify", "--set", f"profile.vendortype={vendor_type}", "-e", "has-readme", "/charts"], capture_output=True)
        else:
            out = subprocess.run([chart_verifier_binary_path, "verify", "--set", f"profile.vendortype={vendor_type}", "/charts"], capture_output=True)
    elif tar_exists:
        dn = os.path.join(os.getcwd(), "charts", category,
                          organization, chart, version)
        if os.path.exists(report_path):
            out = subprocess.run([chart_verifier_binary_path, "verify", "--set", f"profile.vendortype={vendor_type}", "-e", "has-readme", f"/charts/{chart}-{version}.tgz"], capture_output=True)
        else:
            out = subprocess.run([chart_verifier_binary_path, "verify", "--set", f"profile.vendortype={vendor_type}", f"/charts/{chart}-{version}.tgz"], capture_output=True)
    else:
        return
