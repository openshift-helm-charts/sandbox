...
def match_name_and_version(directory, category, organization, chart, version,generated_report_path):
    ...
    if os.path.exists(submitted_report_path):
        ...
        if os.path.exists(generated_report_path):
            report_chart = report_info.get_report_chart(reoprt_path=generated_report_path)
...
def check_report_success(directory, api_url, report_path, report_info_path, version):
    ...
    report_profile_type = report_metadata["profileType"]

    if report_profile_type != vendor_type:
        msg = f"[ERROR] Report profile type '{report_profile_type}' doesn't match the vendor type in the directory path: '{vendor_type}'"
        write_error_log(directory, msg)
        sys.exit(1)
