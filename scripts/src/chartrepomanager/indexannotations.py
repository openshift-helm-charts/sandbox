...
def getOCPVersions(kubeVersion):
    ...
    for kubeVersionKey in kubeOpenShiftVersionMap:
        ...
        if coercedKubeVersionKey in semantic_version.NpmSpec(checkKubeVersion):
            ...
            coercedMinOCP = semantic_version.Version.coerce(minOCP)
            coercedMaxOCP = semantic_version.Version.coerce(maxOCP)

            if minOCP == "" or coercedMinOCP > coercedOCPVersionValue:
                minOCP = kubeOpenShiftVersionMap[kubeVersionKey]
            if maxOCP == "" or coercedMaxOCP < coercedOCPVersionValue:
