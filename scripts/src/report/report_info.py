...
def _get_report_info(report_path, report_info_path,info_type, profile_type, profile_version):
    ...
    if report_info_path and len(report_info_path) > 0:
        print("\n[INFO] Using existing report info\n")
        ...
        if os.environ.get("VERIFIER_IMAGE"):
            print("\n[INFO] Generate report info using docker\n")
            ...
            print("\n[INFO] Generate report info using chart-verifier on path\n")
            if set_values:
                out = subprocess.run(["chart-verifier",command,info_type,"--set",set_values,os.path.basename(report_path)],capture_output=True)
            else:
                out = subprocess.run(["chart-verifier",command,info_type,os.path.basename(report_path)],capture_output=True)
