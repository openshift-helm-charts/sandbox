...
def _get_report_info(report_path, info_type, profile_type, profile_version):
    ...
    if "chart-verifier" not in openshift_tools_installer_output:
        print("[ERROR] missing 'chart-verifier' binary")
        sys.exit(1)
    ...
    command = [chart_verifier_binary_path, "report", info_type, "/charts/"+os.path.basename(report_path)]
    ...
    print(f'[INFO] calling chart-verifier: {command}, report directory: {report_directory}')
