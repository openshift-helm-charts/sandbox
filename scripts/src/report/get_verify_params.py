...
def generate_verify_options(directory: str, s: validate.Submission):
    ...
    if s.report.found:
        ...
        flags = f"{flags} -e has-readme"  # Why check has readme only if report + source is included. Not if report only, or source only
        # Same with report_needed: False if report only, True if source (with or without repport)
    ...
    if s.source.found and not s.tarball.found:
        ...
        msg = "[ERROR] Both chart source dierctory and tarball should not exist"
