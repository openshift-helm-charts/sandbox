...
def main():
    ...
    parser.add_argument(
        ...
        dest="categories",
        required=True,
        help="Allowed category for this chart (community, partners, or redhat). Can be specified mutliple times",
        action="append")
    ...
    # Parse the modified file to calssify the file
    try:
    ...
    if len(s.modified_owners) == 1 and s.chart.category in args.categories:
        ...
        print(f"The file in the PR is not a {'/'.join(x for x in args.categories)} OWNERS file")
        gitutils.add_output("merge_pr", "false")
        gitutils.add_output("msg", f"ERROR: PR does not include a {'/'.join(x for x in args.categories)} OWNERS file.")
        sys.exit(30)
